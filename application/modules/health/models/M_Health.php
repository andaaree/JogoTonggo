...
class M_Health extends CI_Model
{
    ...
    {
        $uri = substr($this->uri->uri_string(), 0, 6);
        $reg = $this->session->userdata('v_reg');
    ...
    {
        $qry = $this->db->query("SELECT DISTINCT rw_res.quest_id,quest_type,quest_full FROM rw_res,v_form_health WHERE rw_res.quest_id=v_form_health.quest_id ORDER BY quest_type DESC;");
