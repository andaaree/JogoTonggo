<div class="content-wrapper">
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-12">
                    <h1 class="m-0 text-dark">Dashboard Sosial & Keamanan Provinsi</h1>
                </div><!-- /.col -->
    ...
    foreach ($out as $o) {
        if ($o['quest_type'] == 'opt') { ?>
            ...
            $(document).ready(function() {
                ...
                $.ajax({
                    type: "GET",
                    url: link + "socials/AllLabel/" + "<?= $this->encrypt->encode($o['quest_id']); ?>",
                    dataType: "JSON",
                    success: function(data) {
                        ...
                        pl.forEach(element => {
                            $.ajax({
                                type: "GET",
                                dataType: "JSON",
                                url: link + "socials/AllData?label=" + element + "&id=" + "<?= $this->encrypt->encode($o['quest_id']); ?>",
                                success: function(rs) {
                                    rs = rs.toString();
                        ...
                        $.getJSON("socials/AllData").done(function() {
                            var donutData = {
                                labels: pl,
                                datasets: [{
                                    data: tot,
        ...
        <?php } elseif ($o['quest_type'] == 'total') { ?>
            ...
            $.ajax({
                type: "GET",
                url: link + "socials/fetchTot/<?= $this->encrypt->encode($o['quest_id']); ?>",
                success: function(data) {
                    $('.<?= MD5('tot-'.$o['quest_id']); ?>').html(data);
                }
            });
