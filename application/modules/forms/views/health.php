<div class="content-wrapper">
                    ...
                    <div class="card">
                        <div class="card-header mt-3">
                            <h3 class="card-title">
                                ...
                                Daftar Pertanyaan Kesehatan
                            </h3>
                        </div>
                        <!-- /.card-header -->
                        <div class="card-body">
                            <form method="POST" action="<?= base_url('') ?>forms/saveH">
                                <?php
                                $no = 1;
                                foreach ($ans as $o) { ?>
                                    ...
                                    <?php } elseif ($o['quest_type'] == 'essay') { ?>
                                        <div class="form-group">
                                            <input class="form-control" type="text" name="ans<?= $no; ?>" placeholder="<?php if (strpos($o['quest_full'], 'sedia') != false) {
                                                                                                                            echo "Pilih : Analgesik / Antihistamin / Dekongestan / Ekspektoran / Antitusif";
                                                                                                                        } ?>">
                                        </div>
                                <?php }
                                    $no++;
    ...
    <?php foreach ($ans as $o) { ?>
        <script type="text/javascript">
            $.ajax({
                type: "GET",
                url: link + "forms/health/valid/" + "<?= $o['quest_id'] ?>",
