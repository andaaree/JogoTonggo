<div class="content-wrapper">
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-12">
                    <h1 class="m-0 text-dark">Dashboard Kesehatan Provinsi</h1>
                </div><!-- /.col -->
                    ...
                    <div class="col-lg-3 col-sm-6">
                        <div class="card">
                            <div class="card-body bg-gradient-yellow text-center text-lg">
                                <p><?= $o['quest_full']; ?></p>
                                <p class="tot-<?= MD5($o['quest_id']); ?>">0</p>
                            </div>
                        </div>
                    </div>
            <?php    }
    ...
    foreach ($out as $o) {
        if ($o['quest_type'] == 'opt') { ?>
            ...
            $(document).ready(function() {
                ...
                $.ajax({
                    type: "GET",
                    url: link + "health/AllLabel/" + "<?= $this->encrypt->encode($o['quest_id']); ?>",
                    dataType: "JSON",
                    success: function(data) {
                        ...
                        pl.forEach(element => {
                            $.ajax({
                                type: "GET",
                                dataType: "JSON",
                                url: link + "health/AllData?label=" + element + "&id=" + "<?= $this->encrypt->encode($o['quest_id']); ?>",
                                success: function(rs) {
                                    rs = rs.toString();
                        ...
                        $.getJSON("health/AllData").done(function() {
                            var donutData = {
                                labels: pl,
                                datasets: [{
                                    data: tot,
        ...
        <?php } elseif ($o['quest_type'] == 'total') { ?>
            ...
            $.ajax({
                type: "GET",
                url: link + "health/fetchTot/<?= $this->encrypt->encode($o['quest_id']); ?>",
                success: function(data) {
                    $('.tot-<?= MD5($o['quest_id']); ?>').html(data);
                }
            });
