...
class M_Ent extends CI_Model
{
    public function getTotKab()
    {
        $uri = substr($this->uri->uri_string(), 0, 9);
    ...
    {
        $qry = $this->db->query("SELECT DISTINCT cat_name,rw_res.quest_id,quest_type,quest_full FROM rw_res,v_form_ent WHERE rw_res.quest_id=v_form_ent.quest_id ORDER BY quest_type DESC;");
